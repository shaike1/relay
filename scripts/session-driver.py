        ...
        with open(QUEUE_FILE) as f:
            for line in f:
                ...
                try:
                    ...
                    # Skip scheduled system messages (via=scheduled)
                    if msg.get("via") == "scheduled":
                        log.debug(f"Skipping scheduled system message mid={mid}: {msg.get('text','')[:60]!r}")
                        continue
                    # Skip noise messages using unified filter
                    _txt = msg.get("text", "")
                    if is_noise(_txt):
...
NOISE_TOKENS: tuple[str, ...] = (
    # Queue-level noise (also filtered in read_pending_messages)
    ...
    "לא הגיב", "מפעיל מחדש",
    # Response-level noise
    "Claude Code v", "API Usage Billing", "auto-route",
    "checkpoint", "Session Checkpoint", "ctrl+o to expand",
...
def is_noise(text: str) -> bool:
    """Return True if text is internal noise that should never be sent to Telegram."""
    if not text or not text.strip():
        return True
    return any(t in text for t in NOISE_TOKENS)
        ...
        try:
            ...
            for msg in messages:
                ...
                if cleaned_response:
                    ...
                    # Final noise guard before sending
                    if is_noise(cleaned_response):
                        log.warning(f"Blocked noise from send_message mid={mid}: {cleaned_response[:60]!r}")
                    else:
